...
def create_dataframe(csv_file_path, date, market_open, market_close):
    ...
    intraday_trades = trades.select(
        where=(
                (kx.Column('timestamp').cast('date')) == date
        )
    )
    ...
    filtered_intraday_trades = intraday_trades.select(
        where=(
                (kx.Column('timestamp') >= market_open) &
                (kx.Column('timestamp') <= market_close)
        )
    )

    # Execute a qSQL query using xbar to bucket the minutes into hours
    trades_table = filtered_intraday_trades.select(kx.Column('trade_sum', value=kx.Column('i').sum()),
                                                   by=kx.Column('time', value=kx.Column('timestamp').minute.xbar(60)))

    # Other way to rename columns
    trades_table = filtered_intraday_trades.select(kx.Column('i').sum().name('trade_sum'),
                                                   by=kx.Column('timestamp').minute.xbar(60))
